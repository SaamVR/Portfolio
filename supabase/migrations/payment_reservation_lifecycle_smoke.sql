...
CREATE OR REPLACE FUNCTION pg_temp.create_payment_order(
  _store_id uuid, _client_request_id text, _user_id uuid, _items jsonb,
  _delivery_fee integer, _discount_amount integer, _customer_name text,
  _customer_phone text, _customer_email text, _shipping_address text,
  _shipping_city text, _payment_method text, _notes text, _coupon_code text
)
RETURNS TABLE (
  id uuid, order_number text, subtotal integer, delivery_fee integer,
  total integer, payment_method text, status text, replayed boolean,
  reservation_state text, reservation_expires_at timestamptz
)
LANGUAGE sql
AS $$
  SELECT *
  FROM public.create_store_order_with_payment_lifecycle(
    _store_id, _client_request_id, _user_id, _items,
    'primary', _delivery_fee, _discount_amount,
    _customer_name, _customer_phone, _customer_email,
    _shipping_address, _shipping_city, _payment_method,
    true, lower(trim(_payment_method)) <> 'cod',
    CASE WHEN lower(trim(_payment_method)) = 'bkash' THEN 'bkash' ELSE NULL END,
    NULL, NULL, _notes, _coupon_code
  );
$$;
...
BEGIN
  ...
  INSERT INTO public.site_settings (store_id, key, value) VALUES
    (_store, 'delivery_settings', '{"enabled":false,"primary_zone_aliases":["Dhaka"]}'::jsonb),
    (_store, 'payment_settings', '{"cod_enabled":true}'::jsonb);
  INSERT INTO public.store_payment_connections_secure (store_id, provider, status, revoked_at)
  VALUES (_store, 'bkash', 'connected', NULL);
  ...
  SELECT * INTO _order
  FROM pg_temp.create_payment_order(
  ...
  INSERT INTO public.coupon_codes (id, store_id, code, discount_type, discount_value, max_uses, uses_count)
  VALUES (_coupon, _store, _coupon_code, 'percentage', 10, 1, 0);
  SELECT * INTO _order FROM pg_temp.create_payment_order(
  ...
  SELECT * INTO _order2 FROM pg_temp.create_payment_order(
