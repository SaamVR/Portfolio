...
-- Integration wrapper: authoritative order v3 owns option/delivery/payment/manual
-- payment truth; this function atomically adds the redirect-payment reservation lifecycle.
...
DECLARE
  ...
  _coupon_id uuid;
  _is_redirect_payment boolean :=
    lower(trim(coalesce(_payment_method, ''))) = 'bkash'
    AND lower(trim(coalesce(_payment_provider, ''))) = 'bkash';
BEGIN
  SELECT * INTO _result
...
REVOKE ALL ON FUNCTION public.create_store_order_with_payment_lifecycle(
  uuid, text, uuid, jsonb, text, integer, integer, text, text, text, text, text,
  text, boolean, boolean, text, text, text, text, text
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_store_order_with_payment_lifecycle(
  uuid, text, uuid, jsonb, text, integer, integer, text, text, text, text, text,
  text, boolean, boolean, text, text, text, text, text
) TO service_role;
GRANT ALL ON FUNCTION public.create_store_order_with_payment_lifecycle(
  uuid, text, uuid, jsonb, text, integer, integer, text, text, text, text, text,
  text, boolean, boolean, text, text, text, text, text
) TO postgres;

COMMENT ON FUNCTION public.create_store_order_with_payment_lifecycle(
  uuid, text, uuid, jsonb, text, integer, integer, text, text, text, text, text,
  text, boolean, boolean, text, text, text, text, text
) IS
  'Composes authoritative v3 order creation with finite redirect-payment reservation authority and exact-once release semantics.';
