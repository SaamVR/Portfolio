...
BEGIN
  ...
  SELECT pg_get_functiondef(p.oid) INTO _manual_claim_function
  FROM pg_proc p
  JOIN pg_namespace n ON n.oid = p.pronamespace
  WHERE n.nspname = 'public'
    AND p.proname = 'claim_storefront_manual_payment_reference'
  LIMIT 1;
  IF _manual_claim_function IS NULL
     OR _manual_claim_function NOT ILIKE '%NEW.manual_payment_reference%'
     OR _manual_claim_function ILIKE '%NEW.notes%' THEN
    RAISE EXCEPTION 'live manual payment claim authority is not bound to structured order identity';
  END IF;
