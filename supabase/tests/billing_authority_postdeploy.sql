...
DECLARE
  ...
  v_authorized_select_policy integer;
...
BEGIN
  ...
  SELECT count(*) INTO v_authorized_select_policy
  FROM pg_policies
  WHERE schemaname = 'public'
    AND tablename = 'store_invoices'
    AND cmd = 'SELECT'
    ...
    AND (
      position('can_manage_store' in COALESCE(qual, '')) > 0
      OR position('can_administer_store' in COALESCE(qual, '')) > 0
    );
  ...
  IF v_anon_privileges <> 0
     ...
     OR v_select_policies <> 1
     OR v_authorized_select_policy <> 1
    ...
    RAISE EXCEPTION USING
      ...
      DETAIL = format(
        'anon_privileges=%s auth_nonselect=%s auth_select=%s service_dml=%s mutation_policies=%s select_policies=%s authorized_select_policy=%s invoice_trigger=%s plan_triggers=%s unique_index=%s identity_constraint=%s provider_method_mismatch=%s duplicate_groups=%s',
        v_anon_privileges, v_authenticated_nonselect, v_authenticated_select,
        v_service_role_dml, v_mutation_policies, v_select_policies, v_authorized_select_policy,
