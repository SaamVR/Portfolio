...
const Coupons = () => {
  ...
  const createCoupon = useMutation({
    mutationFn: async () => {
      ...
      if (editId) {
        const { error } = await supabase.from("coupon_codes" as any).update(payload).eq("id", editId);
        if (error) throw error;
      } else {
  ...
  const toggleActive = useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      const { error } = await supabase.from("coupon_codes" as any).update({ is_active }).eq("id", id);
      if (error) throw error;
    },
  ...
  const deleteCoupon = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("coupon_codes" as any).delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
