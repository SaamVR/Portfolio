...
const AdminLayout = ({ children }: { children?: React.ReactNode }) => {
  ...
  const loginRedirectTarget = `/admin/login?next=${encodeURIComponent(`${location.pathname}${location.search || ""}`)}`;
  ...
  const isBasicBuilder = location.pathname.includes("/admin/page-builder/basic");
  ...
  return (
    <div className="flex min-h-screen bg-background">
      {!isBasicBuilder && <AdminSidebar />}
      <main className={cn("flex-1 overflow-y-auto", !isBasicBuilder && "pb-20 md:pb-0")}>
        ...
        <div
          className={cn(
            !isBasicBuilder && "container mx-auto px-4 py-6 md:px-8 md:py-8",
            !isBasicBuilder && isCmsWorkspace ? "max-w-[1560px]" : (!isBasicBuilder ? "max-w-6xl" : "h-full"),
          )}
        >
          {children}
        </div>
      </main>
      {!isBasicBuilder && <AdminMobileNav onOpenCommand={() => setCommandOpen(true)} />}
