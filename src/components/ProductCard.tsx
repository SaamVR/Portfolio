  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-border bg-card transition-all duration-500 ease-spring-2 hover:border-primary/40 hover:-translate-y-1.5 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)]">
      <Link
        ...
        aria-label={`View ${product.name} - ${product.colors[0]} - BDT ${product.price}`}
      >
        <div className="relative aspect-[4/5] overflow-hidden bg-secondary">
          ...
          {product.badge && (
            <span
              className={cn(
                "absolute left-3 top-3 z-10 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-widest shadow-sm",
          ...
          {product.featured && !product.badge && (
            <span className="absolute left-3 top-3 rounded-full bg-primary/90 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-primary-foreground shadow-sm backdrop-blur-sm">
              Featured
            </span>
          )}
          {product.stock !== undefined && product.stock > 0 && product.stock <= 5 && (
            <span className="absolute left-3 bottom-3 z-10 rounded-md bg-destructive/90 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-destructive-foreground animate-pulse">
              Only {product.stock} left!
            </span>
          )}
          <span className="absolute right-3 top-3 rounded-full border border-white/10 bg-background/60 px-3 py-1 text-[10px] font-semibold tracking-wider text-muted-foreground backdrop-blur-md">
            {product.type}
          </span>
        </div>
        <div className="p-5">
          ...
          <div className="mt-2.5 flex items-center gap-2.5">
            <p className="font-heading text-lg font-bold text-primary transition-colors duration-300 group-hover:text-accent">BDT {product.price}</p>
            {product.originalPrice && (
              <p className="font-heading text-sm text-muted-foreground line-through opacity-70">BDT {product.originalPrice}</p>
            )}
          </div>
          {product.colors.length > 1 && (
            <div className="mt-3.5 flex items-center gap-1.5">
              ...
              {product.colors.length > 5 && <span className="ml-1 text-[10px] font-medium text-muted-foreground">+{product.colors.length - 5}</span>}
            </div>
          )}
        </div>
      </Link>
      ...
      <button
        ...
        className={cn(
          ...
          wishlisted
            ...
            : "border-border bg-background/80 text-muted-foreground backdrop-blur-sm hover:border-primary/30 hover:text-primary",
        )}
      >
      ...
      <div className="absolute bottom-[calc(theme(spacing.5)+6.5rem)] left-1/2 z-10 hidden -translate-x-1/2 translate-y-6 items-center gap-2 opacity-0 transition-all duration-500 ease-spring-2 group-hover:translate-y-0 group-hover:opacity-100 lg:flex">
        {onQuickView && (
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-full border border-border bg-background/90 text-foreground shadow-lg backdrop-blur-md transition-transform hover:scale-105 hover:bg-background"
          >
        ...
        <button
          ...
          className="flex h-10 items-center gap-2 rounded-full bg-primary/95 px-5 text-[13px] font-bold uppercase tracking-wider text-primary-foreground shadow-lg backdrop-blur-sm transition-transform hover:scale-105 hover:bg-primary"
        >
