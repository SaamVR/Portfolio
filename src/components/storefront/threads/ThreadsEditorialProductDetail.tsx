...
const assuranceItems = [
  { icon: Truck, title: "Tracked delivery", text: "Delivery choices are confirmed at checkout." },
  { icon: RotateCcw, title: "Easy returns", text: "Eligible orders follow the store return policy." },
  { icon: ShieldCheck, title: "Secure checkout", text: "Protected payment and order processing." },
];

export function ThreadsEditorialProductDetail({ product }: { product: Product }) {
      ...
      <section className="border-b border-border/60 bg-secondary/35">
        <div className="mx-auto grid max-w-[1320px] grid-cols-1 divide-y divide-border/60 px-4 sm:px-6 md:grid-cols-3 md:divide-x md:divide-y-0 md:px-8">
          {assuranceItems.map(({ icon: Icon, title, text: copy }) => (
            <div key={title} className="flex items-start gap-3 py-6 md:px-6 md:py-7 first:md:pl-0 last:md:pr-0">
              ...
              <div>
                ...
                <p className="mt-1 text-[8px] leading-4 text-muted-foreground md:text-[9px]">{copy}</p>
              </div>
            </div>
          ))}
        </div>
            ...
            {details.length > 0 ? (
              <dl className="mt-7 grid gap-x-8 gap-y-0 border-t border-border sm:grid-cols-2">
                {details.map(([label, value]) => (
                  <div key={label} className="grid grid-cols-[78px_1fr] gap-3 border-b border-border py-4 text-[9px] leading-4 md:text-[10px]">
