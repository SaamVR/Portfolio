...
function FashionHero({ block }: { block: StorePageBlock }) {
  ...
  return (
    <section className="relative min-h-[72svh] overflow-hidden bg-[#deded8] md:min-h-[82vh]">
      ...
      <div className="relative mx-auto flex min-h-[72svh] max-w-[1500px] items-end px-5 pb-10 pt-24 text-white md:min-h-[82vh] md:px-8 md:pb-14 lg:px-12">
        <div className="max-w-3xl">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            {cta ? <Link href={ctaLink} className="inline-flex min-h-12 items-center justify-center bg-white px-6 text-xs font-semibold uppercase tracking-[0.14em] text-black transition hover:bg-[#e7e7e2]">{cta}</Link> : null}
  ...
  return (
    <section className="mx-auto max-w-[1500px] px-5 py-14 md:px-8 md:py-24 lg:px-12">
      <div className="mb-8 flex items-end justify-between gap-6 md:mb-12">
        <div><p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/45">{asString(props.tagline) || "Collections"}</p><h2 className="text-3xl font-semibold tracking-[-0.045em] md:text-5xl">{asString(props.title) || "Shop by collection"}</h2></div>
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-4">
        {items.slice(0, 5).map((item, index) => (
          <Link key={`${item.name}-${index}`} href={`${shop}?category=${encodeURIComponent(item.value || item.name)}`} className={`group relative overflow-hidden bg-[#e8e8e4] ${index === 0 ? "col-span-2 aspect-[16/10] md:col-span-2 md:row-span-2 md:aspect-auto" : "aspect-[4/5]"}`}>
...
function FashionProducts({ block }: { block: StorePageBlock }) {
  ...
  return (
    <section className="mx-auto max-w-[1500px] px-5 py-14 md:px-8 md:py-24 lg:px-12">
      <div className="mb-8 flex items-end justify-between gap-6 border-b border-black/10 pb-5 md:mb-10"><div><p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/45">{asString(props.tagline) || "New season"}</p><h2 className="text-3xl font-semibold tracking-[-0.045em] md:text-5xl">{asString(props.title) || "New arrivals"}</h2></div><Link href={storefrontPath("/shop", store?.slug)} className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em]">View all <ArrowRight className="h-4 w-4" /></Link></div>
...
function FashionPromo({ block }: { block: StorePageBlock }) {
  const store = useOptionalStore();
  const props = block.props as Record<string, unknown>;
  return <section className="bg-[#171717] text-white"><div className="mx-auto grid max-w-[1500px] gap-7 px-5 py-12 md:grid-cols-[1fr_auto] md:items-end md:px-8 md:py-16 lg:px-12"><div><p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-white/50">{asString(props.badgeText) || "Edit"}</p><h2 className="max-w-[15ch] text-3xl font-semibold leading-[0.95] tracking-[-0.045em] md:text-5xl">{asString(props.title) || "A new season, considered."}</h2>{asString(props.subtitle) ? <p className="mt-4 max-w-2xl text-sm leading-6 text-white/65">{asString(props.subtitle)}</p> : null}</div>{asString(props.ctaText) ? <Link href={storefrontPath(asString(props.ctaLink) || "/shop", store?.slug)} className="inline-flex min-h-11 items-center gap-2 border-b border-white pb-1 text-xs font-semibold uppercase tracking-[0.12em]">{asString(props.ctaText)} <ArrowRight className="h-4 w-4" /></Link> : null}</div></section>;
}
...
function FashionSocial({ block }: { block: StorePageBlock }) {
  ...
  return <section className="py-14 md:py-24"><div className="mx-auto max-w-[1500px] px-5 md:px-8 lg:px-12"><div className="mb-8"><p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/45">Lookbook</p><h2 className="text-3xl font-semibold tracking-[-0.045em] md:text-5xl">{asString(props.title) || "Seen outside the studio"}</h2>{asString(props.subtitle) ? <p className="mt-3 max-w-xl text-sm text-black/55">{asString(props.subtitle)}</p> : null}</div><div className="grid grid-cols-2 gap-1 md:grid-cols-4">{images.slice(0, 4).map((src, index) => <div key={`${src}-${index}`} className="relative aspect-[4/5] overflow-hidden bg-[#eee]"><SafeStorefrontImage src={src} alt={`Lookbook ${index + 1}`} fill className="object-cover" /></div>)}</div></div></section>;
}
...
function FashionTestimonials({ block }: { block: StorePageBlock }) {
  ...
  return (
    <section className="bg-[#f0f1f2] py-14 md:py-20">
      <div className="mx-auto max-w-[1500px] px-5 md:px-8 lg:px-12">
        <div className="mb-9 max-w-3xl">
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/45">Customer notes</p>
          <h2 className="text-3xl font-semibold tracking-[-0.045em] md:text-5xl">{asString(props.title) || "What customers say"}</h2>
          {asString(props.subtitle) ? <p className="mt-3 text-sm text-black/50">{asString(props.subtitle)}</p> : null}
        </div>
        <div className="grid border-t border-black/15 md:grid-cols-3">
          {reviews.slice(0, 3).map((review, index) => {
            ...
            return (
              <article key={`${name}-${index}`} className="border-b border-black/15 py-6 md:border-b-0 md:border-r md:px-7 first:md:pl-0 last:md:border-r-0">
                {Number.isFinite(rating) && rating >= 1 && rating <= 5 ? <p className="text-[11px] font-semibold tracking-[0.12em] text-black/45">{rating.toFixed(0)}/5</p> : null}
                {text ? <p className="mt-4 max-w-sm text-base leading-7 text-black/80">“{text}”</p> : null}
                {name ? <p className="mt-5 text-[10px] font-semibold uppercase tracking-[0.16em] text-black/45">{name}</p> : null}
              </article>
            );
          })}
        </div>
...
function FashionStory({ block }: { block: StorePageBlock }) {
  ...
  return (
    <section className="border-y border-black/10 py-14 md:py-24">
      <div className="mx-auto grid max-w-[1500px] gap-9 px-5 md:grid-cols-2 md:items-center md:px-8 lg:gap-16 lg:px-12">
        {image ? <div className="relative aspect-[4/5] overflow-hidden bg-[#ecece8]"><SafeStorefrontImage src={image} alt={asString(props.imageAlt) || asString(props.title) || "Brand story"} fill className="object-cover" /></div> : null}
        <div className={image ? "" : "md:col-span-2 md:max-w-4xl"}>
          <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/45">{asString(props.eyebrow) || "Our story"}</p>
          ...
          {paragraphs.length > 0 ? <div className="mt-6 max-w-2xl space-y-4 text-sm leading-7 text-black/58 md:text-base">{paragraphs.slice(0, 6).map((paragraph, index) => <p key={`${paragraph.slice(0, 24)}-${index}`}>{paragraph}</p>)}</div> : null}
        </div>
      </div>
    </section>
  );
    ...
    <section className="border-t border-black/10 py-14 md:py-20">
      <div className="mx-auto max-w-[1500px] px-5 md:px-8 lg:px-12">
        <div className="mb-8 flex items-end justify-between border-b border-black/10 pb-5">
          <div><p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/45">{asString(props.tagline) || "Curated for you"}</p><h2 className="text-3xl font-semibold tracking-[-0.045em] md:text-4xl">{asString(props.title) || "More to explore"}</h2></div>
...
function FashionCountdown({ block }: { block: StorePageBlock }) {
  ...
  return (
    <section className="bg-[#171717] py-12 text-white md:py-16">
      <div className="mx-auto grid max-w-[1500px] gap-8 px-5 md:grid-cols-[1fr_auto] md:items-end md:px-8 lg:px-12">
        <div><p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-white/45">Limited release</p><h2 className="max-w-[14ch] text-3xl font-semibold leading-[0.95] tracking-[-0.045em] md:text-5xl">{asString(props.title) || "The next drop is almost here"}</h2>{asString(props.subtitle) ? <p className="mt-4 max-w-xl text-sm leading-6 text-white/60">{asString(props.subtitle)}</p> : null}</div>
        <div>
          {remaining !== null ? <div className="grid grid-cols-4 gap-5 border-y border-white/20 py-4 text-center"><div><b className="block text-2xl font-medium">{days}</b><span className="text-[9px] uppercase tracking-[0.16em] text-white/45">Days</span></div><div><b className="block text-2xl font-medium">{String(hours).padStart(2, "0")}</b><span className="text-[9px] uppercase tracking-[0.16em] text-white/45">Hours</span></div><div><b className="block text-2xl font-medium">{String(minutes).padStart(2, "0")}</b><span className="text-[9px] uppercase tracking-[0.16em] text-white/45">Min</span></div><div><b className="block text-2xl font-medium">{String(seconds).padStart(2, "0")}</b><span className="text-[9px] uppercase tracking-[0.16em] text-white/45">Sec</span></div></div> : null}
          {asString(props.ctaText) ? <Link href={storefrontPath(asString(props.ctaLink) || "/shop", store?.slug)} className="mt-5 inline-flex min-h-11 items-center gap-2 border-b border-white pb-1 text-xs font-semibold uppercase tracking-[0.12em]">{asString(props.ctaText)} <ArrowRight className="h-4 w-4" /></Link> : null}
        </div>
      </div>
    </section>
  );
...
function FashionRecentlyViewed({ block }: { block: StorePageBlock }) {
  ...
  const props = block.props as Record<string, unknown>;
  return (
    <section className="border-t border-black/10 py-14 md:py-20"><div className="mx-auto max-w-[1500px] px-5 md:px-8 lg:px-12"><div className="mb-8"><p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/45">Continue browsing</p><h2 className="text-3xl font-semibold tracking-[-0.04em]">{asString(props.title) || "Recently viewed"}</h2></div><div className="grid grid-cols-2 gap-x-3 gap-y-10 md:grid-cols-4 md:gap-x-5">{recent.map((product) => <FashionV3ProductCard key={product.id} product={product} />)}</div></div></section>
  );
}
...
function FashionFaq({ block }: { block: StorePageBlock }) {
  ...
  return <section className="border-t border-black/10 py-14 md:py-20"><div className="mx-auto grid max-w-[1200px] gap-10 px-5 md:grid-cols-[0.75fr_1.25fr] md:px-8"><div><p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/45">Need to know</p><h2 className="text-3xl font-semibold tracking-[-0.04em] md:text-4xl">{asString(props.title) || "Before you order"}</h2></div><div className="border-t border-black/15">{entries.map((entry, index) => { const q = asString(entry.question) || asString(entry.q); const a = asString(entry.answer) || asString(entry.a); return <div key={`${q}-${index}`} className="border-b border-black/15"><button type="button" onClick={() => setOpen(open === index ? null : index)} className="flex w-full items-center justify-between gap-4 py-5 text-left text-sm font-medium"><span>{q}</span><ChevronDown className={`h-4 w-4 transition ${open === index ? "rotate-180" : ""}`} /></button>{open === index ? <p className="pb-5 pr-8 text-sm leading-6 text-black/55">{a}</p> : null}</div>; })}</div></div></section>;
}
...
function FashionTrust({ block }: { block: StorePageBlock }) {
  ...
  return <section className="border-y border-black/10 bg-[#f4f4f0]"><div className="mx-auto grid max-w-[1500px] divide-y divide-black/10 px-5 md:grid-cols-3 md:divide-x md:divide-y-0 md:px-8 lg:px-12">{badges.slice(0, 3).map((badge, index) => <div key={`${asString(badge.label)}-${index}`} className="py-8 md:px-8 md:py-10 first:md:pl-0"><Check className="mb-4 h-4 w-4" /><h3 className="text-sm font-semibold">{asString(badge.label)}</h3>{asString(badge.description) ? <p className="mt-2 max-w-xs text-xs leading-5 text-black/50">{asString(badge.description)}</p> : null}</div>)}</div></section>;
}
