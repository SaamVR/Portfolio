...
import { getSectionStyleLibraryEntries } from "@/lib/cms/storefront-platform/variants/section-style-library";
...
export default function SectionStylesWorkspace() {
  ...
  const applyStyle = async () => {
    if (!activeStoreId || !workspace || !selectedBlock || !selectedEntry || !selectedEntry.compatible) return;
    setApplying(true);
    try {
      const { error: updateError } = await (supabase as any)
        .from("store_page_blocks")
        .update({ layout_variant: selectedEntry.definition.id })
        .eq("store_id", activeStoreId)
        .eq("id", selectedBlock.id);
      if (updateError) throw updateError;
      setWorkspace((current) => current ? {
        ...current,
        blocks: current.blocks.map((block) => block.id === selectedBlock.id ? { ...block, layoutVariant: selectedEntry.definition.id } : block),
      } : current);
    ...
    <div className="mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mt-5 flex gap-2 overflow-x-auto pb-2">
        {supportedBlocks.map((block, index) => (
          <button key={block.id} type="button" onClick={() => { setSelectedBlockId(block.id); setSelectedStyleId(""); }} className={cn("min-h-11 shrink-0 rounded-full border px-4 text-sm font-medium", selectedBlock.id === block.id ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background hover:border-primary/40")}>
        ...
        <div>
          <div className="mb-3 flex items-end justify-between gap-3">
            ...
            <div className="flex rounded-xl border border-border bg-muted/30 p-1">
              <Button type="button" size="sm" variant={previewMode === "desktop" ? "secondary" : "ghost"} className="min-h-9 gap-1.5" onClick={() => setPreviewMode("desktop")}><Monitor className="h-3.5 w-3.5" />Desktop</Button>
              <Button type="button" size="sm" variant={previewMode === "mobile" ? "secondary" : "ghost"} className="min-h-9 gap-1.5" onClick={() => setPreviewMode("mobile")}><Smartphone className="h-3.5 w-3.5" />Mobile</Button>
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {entries.map((entry) => {
              ...
              return (
                <button key={entry.definition.id} type="button" onClick={() => setSelectedStyleId(entry.definition.id)} className={cn("rounded-2xl border p-2 text-left transition hover:-translate-y-0.5 hover:shadow-md", active ? "border-primary bg-primary/5 ring-1 ring-primary/20" : "border-border bg-card", !entry.compatible && "opacity-60")}>
                  <div className="relative">
                    ...
                    <div className="absolute left-2 top-2 flex gap-1">
                      {entry.recommended ? <Badge className="gap-1 text-[10px]"><Sparkles className="h-3 w-3" />Recommended</Badge> : null}
