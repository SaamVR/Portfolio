...
test("cart restore distinguishes unavailable products from legitimate zero-price products", () => {
  ...
  assert.match(source, /const authoritativePrice = commercialOptions\.length > 0/);
  assert.match(source, /: Number\(prod\.price \?\? 0\);/);
  assert.match(source, /if \(authoritativePrice === null \|\| authoritativePrice < 0\) return \[\];/);
  assert.match(source, /price: authoritativePrice/);
