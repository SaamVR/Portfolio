...
function buildFaqs(siteName: string) {
  return [
    ["Are the prices on this page current?", `The first server-rendered plan catalog is loaded from the same public cms_plans catalog used by the product. If the catalog cannot be read, ${siteName} falls back to the documented Free fallback instead of inventing paid prices.`],
    ...
    ["Which courier providers are connected?", `Courier workflows are capability- and configuration-dependent. ${siteName} distinguishes not configured, configured, verified, and attention-needed states instead of treating a provider name as proof of a working connection.`],
    ...
    ["Are external service charges included?", `The plan cards describe ${siteName} billing. Payment, courier, messaging, domain, or other external-provider charges and terms are separate when they apply.`],
  ] as const;
}
...
export async function generateMetadata() {
  ...
  return {
    ...
    description: `Current ${siteName} plan catalog, trial, store-limit, and integration disclosures.`,
  };
}

export default async function PlansPage() {
  const [planCatalog, identity] = await Promise.all([loadPublicPlanCatalog(), getPlatformRuntimeIdentity()]);
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <section className="border-b border-border/70 px-5 py-14 text-center lg:px-8">
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">{identity.siteName} plans</p>
