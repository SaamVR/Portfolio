...
function sampleCamera(name, p){
  const e = ease(p);
  const mechanismMove = ease(clamp01((p - .18) / .60));
  ...
  switch(name){
    ...
    case 'interaction':
      return {pos:lerp3([.28,.06,baseZ-.54],[-.15,.02,baseZ-.68],e),look:[0,-.05,0],fov:30,scale:mobile?.78:.90,model:[mobile?0:4.15,mobile?.48:-.30,0],yaw:0};
    case 'closing':
