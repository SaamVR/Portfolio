...
def test_v3_real_market_benchmarks_replace_fake_specs():
    for phrase in [
        "Sony WH-1000XM6",
    ...
    for source in [
        ...
        "support.sennheiser-hearing.com/hc/en-se/articles/37791406604189-MOMENTUM-4-Wireless-Specifications",
...
def test_v3_removes_fake_notify_demo():
    for token in ['id="notifyConcept"','id="notifyPanel"',"Preview notify flow","Email preview","Demo only / no data"]:
...
def test_v3_product_copy_uses_practical_daily_scenarios():
    for phrase in [
        "Commute",
        "Office",
        "Travel",
...
def test_v3_product_facts_becomes_market_benchmark_and_design_brief():
    assert 'id="productFactsPanel"' in html
    assert "Premium ANC market benchmark" in html
    assert "NOVA design brief" in html
    assert "Design target — not a lab measurement" in html
    for target in ["≤265 g","≥35 h ANC","Bluetooth 5.4 + dual-device multipoint","USB-C digital audio + analog wired fallback"]:
...
def test_v3_design_detail_story_is_sequential():
    assert 'id="designDetailRail"' in html
    for key in ["cushion","headband","controls"]:
        assert f'data-detail-card="{key}"' in html
    assert "Seal + comfort" in html
    assert "Folding hinge" in html
    assert "Tactile controls" in html
    compact=css.replace(" ","").replace("\n","")
    assert 'body[data-range="design"][data-design-detail="cushion"].hotspot[data-hotspot="cushion"]' in compact
    assert 'body[data-range="design"][data-design-detail="headband"].hotspot[data-hotspot="headband"]' in compact
    assert 'body[data-range="design"][data-design-detail="controls"].hotspot[data-hotspot="controls"]' in compact

def test_v3_motion_controller_boundaries_are_explicit():
    app=(ROOT/"app.js").read_text()
    adapter=(ROOT/"runtime/render-adapter.js").read_text()
    hotspot=(ROOT/"interactions/hotspot-controller.js").read_text()
    assert "base.range!=='form'" in app
    assert "clearTourDetailTimer" in app
    assert "cameraTarget" in adapter
    assert "posePhase" in adapter
    assert "currentCameraOffset" in hotspot
    assert "currentTargetOffset" in hotspot
