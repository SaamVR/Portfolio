...
AS $$
BEGIN
  NEW.order_number := 'ORDR-' || to_char(now(), 'YYYYMMDD') || '-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 6));
  RETURN NEW;
END;
$$;
...
INSERT INTO public.site_settings (key, value) VALUES
  ...
  ('contact_page', '{"address": "Dhaka, Bangladesh", "phone": "+880 1XXX-XXXXXX", "email": "hello@example.com"}'::jsonb),
  ...
  ('seo_settings', '{"site_title": "EZComo - Premium Streetwear & Essentials", "meta_description": "Shop premium streetwear, t-shirts, polos, and more from EZComo. Free delivery available.", "og_image": "", "keywords": "streetwear, clothing, fashion, essentials"}'::jsonb)
